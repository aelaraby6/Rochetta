...
export default function RevenueChart({ historical = [], forecast = [], trend = {} }) {
  // SVG Chart Setup
  const combined = [
    ...historical.map((d, index) => ({ val: d.revenue, label: d.date, type: "history", idx: index })),
    ...forecast.map((d, index) => ({ val: d.y, label: d.dateStr, type: "forecast", idx: index + historical.length })),
  ];
  ...
  const points = combined.map((p, i) => {
    const x =
      paddingLeft +
      (i * (chartWidth - paddingLeft - paddingRight)) / (combined.length - 1 || 1);
    const y =
      chartHeight -
      paddingBottom -
  ...
  if (forecastPoints.length > 0 && historyPoints.length > 0) {
    ...
    forecastPath = `M ${lastHistory.x} ${lastHistory.y} ` +
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Last 30 days actual sales plus next 7 days linear regression forecast
          </p>
        </div>
        {trend.trendType && (
      ...
      <div className="overflow-x-auto scrollbar-hide">
        <div className="min-w-[800px] select-none">
          ...
          >
            ...
            {/* Data points markers */}
            {historyPoints
              .filter((_, idx, arr) => idx === 0 || idx === arr.length - 1 || idx % 7 === 0)
              .map((p) => (
                <circle
                  key={p.label}
                  cx={p.x}
            ...
            <g transform={`translate(${paddingLeft}, ${chartHeight - 10})`}>
              ...
              <text x={10} y={0} className="fill-gray-500 text-[11px] font-semibold">
                Actual Revenue (Last 30 days)
              </text>

              <circle cx={220} cy={-4} r={5} fill="#10b981" />
              <text x={230} y={0} className="fill-gray-500 text-[11px] font-semibold">
